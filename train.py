...
with tf.Session() as sess:
    ...
    for i in range(10000):
        batch_date, batch_inf, batch_c, batch_r, zm_train_label, input_length = get_batch(bachsize=batch_size,
                                                                                          data=train_data,
                                                                                          zm2index=zm2index,
                                                                                          word2index=word2index,
                                                                                          truncate_l=truncate_l)
        feed_dic = {model.fzss: batch_date, model.add_inf: batch_inf, model.all_label_c: batch_c,
                    model.all_label_r: batch_r, model.test_zuiming_label: zm_train_label,
                    model.input_length: input_length}
        _, loss, zm_acc, xq_acc = sess.run([opt_op, model.loss, model.acc, model.xq_acc], feed_dict=feed_dic)
        print('loss:',loss,'  ','zm_acc:',zm_acc,'  ','xq_acc:',xq_acc)
        ...
        if (i+1)%100==0:
            batch_date, batch_inf, batch_c, batch_r, zm_test_label, input_length = get_batch(bachsize=batch_size,
                                                                                             ...
                                                                                             truncate_l=truncate_l)
            feed_dic = {model.fzss: batch_date, model.add_inf: batch_inf, model.all_label_c: batch_c,
                        model.all_label_r: batch_r, model.test_zuiming_label: zm_test_label,
                        model.input_length: input_length}
            zm_acc, xq_acc = sess.run([model.acc, model.xq_acc], feed_dict=feed_dic)
            print('zm_acc:',zm_acc,'   ','xq_acc:',xq_acc)
    ...
    for i in range(600):
        batch_date, batch_inf, batch_c, batch_r, zm_test_label, input_length = get_test_data(bachsize=batch_size,
                                                                                             ...
                                                                                             truncate_l=truncate_l)
        feed_dic = {model.fzss: batch_date, model.add_inf: batch_inf, model.all_label_c: batch_c,
                    model.all_label_r: batch_r, model.test_zuiming_label: zm_test_label, model.input_length: input_length}
        zm_acc, xq_acc = sess.run([model.acc, model.xq_acc], feed_dict=feed_dic)
